...
describe 'app' do
  before(:each) do
    ...
    stub_request(:get,
      ...
      .to_return(status: 200, body: File.read("./layer/spec/fixtures/item_10002559.json"))

    stub_request(:get,
      "#{ENV['PLATFORM_API_BASE_URL']}items/sierra-nypl/F16398857")
      .to_return(status: 404, body: File.read("./layer/spec/fixtures/sierra_404.json"))

    stub_request(:get, ENV['NYPL_CORE_S3_BASE_URL'] + "by_catalog_item_type.json")
    .to_return(status: 200, body: File.read("./layer/spec/fixtures/by_catalog_item_type.json"))

    stub_request(:get, ENV['NYPL_CORE_S3_BASE_URL'] + "by_sierra_location.json")
    .to_return(status: 200, body: File.read("./layer/spec/fixtures/by_sierra_location.json"))
  end
