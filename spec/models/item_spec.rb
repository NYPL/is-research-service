describe Item do
  ...
  before(:each) do
    ...
    test_items.each do |test_item|
      stub_request(:get, "#{ENV['PLATFORM_API_BASE_URL']}items/#{test_item[:item].nypl_source}/#{test_item[:item].id}")
        .to_return(status: 200, body: File.read("./spec/fixtures/item_#{test_item[:item].id}.json"))
    end
